...
const Home = () => {
  ...
  const handleAddToCart = (product) => {
    dispatch(addToCart({
      ...
      imageUrl: product.image_main ? `http://localhost:8080/product-images/${product.image_main}` : getImagePlaceholder(product.name, 150, 150),
      sku: product.sku,
      quantity: 1
    }));
              ...
              categories.slice(0, 4).map((category) => (
                <div className="category-card" key={category.id}>
                  <div className="category-image">
                    <img 
                      src={category.image ? `http://localhost:8080/category-images/${category.image}` : getImagePlaceholder(category.name, 150, 150, 'eeeeee', '333333')} 
                      alt={category.name}
                      onError={(e) => {
